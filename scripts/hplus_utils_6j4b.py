...
with open("edges_6jex4bin.pkl", "rb") as f1:
	edges6j4b = pickle.load(f1)
...
def getHplusDf(treename, filename):
	...
	if(flag==1):
		df["weight"] = df["nomWeight_weight_btag"]*df["nomWeight_weight_jvt"]*df["nomWeight_weight_leptonSF"]*df["nomWeight_weight_mc"]*df["nomWeight_weight_norm"]*df["nomWeight_weight_pu"]*df['branch_rew_func_6j']
		df.drop(columns=['nomWeight_weight_btag','nomWeight_weight_jvt','nomWeight_weight_leptonSF','nomWeight_weight_mc','nomWeight_weight_norm','nomWeight_weight_pu','branch_rew_func_6j'], inplace=True)
	elif(flag==2):
		df["weight"] = 1
		...
		df = df.loc[sels]
		df["weight"] = df["nomWeight_weight_btag"]*df["nomWeight_weight_jvt"]*df["nomWeight_weight_leptonSF"]*df["nomWeight_weight_mc"]*df["nomWeight_weight_norm"]*df["nomWeight_weight_pu"]*df['branch_rew_func_6j']*22059247.969726562/185760.6083984375
		df.drop(columns=['nomWeight_weight_btag','nomWeight_weight_jvt','nomWeight_weight_leptonSF','nomWeight_weight_mc','nomWeight_weight_norm','nomWeight_weight_pu','branch_rew_func_6j'], inplace=True)
	else:
