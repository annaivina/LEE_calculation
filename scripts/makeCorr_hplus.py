...
#Correlation matrix will be ewstimated per category region (the categories are considered to be orthogonal to each other) for all mass points (11 points)
...
for mass in sorted(masspoints):
    unique_bins = sorted(df[f"scores_{mass}Bin"].unique())
...
for i, (mass_i, ibin_i) in tqdm(enumerate(all_bins), total=nbins):
    in_i = df[f"scores_{mass_i}Bin"] == ibin_i

    for j, (mass_j, ibin_j) in enumerate(all_bins):
        in_j = df[f"scores_{mass_j}Bin"] == ibin_j
